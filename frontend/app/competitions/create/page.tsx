...
// Define the form data type based on Competition interface
...
export default function CompetitionForm({ params }: { params?: { id: string } }) {
  ...
  const fetchCompetition = async (id: string) => {
    try {
      setLoading(true);
      const competition: Competition = await competitionApi.getCompetition(id);
      
      // Convert the Competition data to form data format
      setFormData({
        ...
        submission_guidelines: competition.submission_guidelines,
        registration_link: competition.registration_link,
        submission_link: competition.submission_link,
        max_team_size: competition.max_team_size,
        judging_criteria: competition.judging_criteria,
        certificate_available: competition.certificate_available,
      });
    } catch (err: any) {
      toast({
        variant: "destructive",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "You must be logged in to create or edit competitions",
      });
      return;
    }
    ...
    try {
      ...
      // Prepare the data with created_by information
      const submissionData = {
        ...
        // The API will automatically set created_by from the authenticated user
        // No need to manually set it here as the backend handles this
      };
      
      if (isEditing && params?.id) {
        await competitionApi.updateCompetition(params.id, submissionData);
        toast({
          title: "Success",
          description: "Competition updated successfully",
        });
      } else {
        await competitionApi.createCompetition(submissionData);
        toast({
          title: "Success",
    ...
    return (
      <div className="container mx-auto py-8">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">You must be logged in to create or edit competitions.</p>
      ...
      <div className="mb-6">
        ...
        {user && (
          <p className="text-sm text-gray-600 mt-2">
            ...
            {user.role === 'company' && user.company_name && ` (${user.company_name})`}
          </p>
        )}
      </div>
